...
namespace Modelo.Aplicacao.Controllers.v1
{
    ...
    public class UsuarioController : TnfController
    {
        ...
        //Retorna um unico usuário por nome
        [HttpGet("usuario")]
        //[Authorize("Bearer")]
        [ProducesResponseType(typeof(Ge_UsuarioDTO), 200)]
        [ProducesResponseType(typeof(ErrorResponse), 400)]
        public async Task<IActionResult> BuscarUsuario(string nomeusuario)
        {
            var retorno = await _Servico.BuscarUmUsuario(nomeusuario);
            if (retorno == null)
            {
                return BadRequest("Erro ao buscar usuario por nome");
            }
            else
            {
        ...
        //Retorna um unico usuário por seq
        [HttpGet("usuario1")]
        //[Authorize("Bearer")]
        [ProducesResponseType(typeof(Ge_UsuarioDTO), 200)]
        [ProducesResponseType(typeof(ErrorResponse), 400)]
        public async Task<IActionResult> BuscarUmUsuarioPorSeq(int seqUsuario)
        {
            var retorno = await _Servico.BuscarUmUsuarioPorSeq(seqUsuario);
            if (retorno == null)
            {
                return BadRequest("Erro ao buscar usuario por seq");
            }
            else
            {
                return CreateResponseOnGet(retorno);
